    ...
    IUSDaiQueuedDepositor
{
    ...
    function _processQueue(
        ...
        while (remainingServiceAmount > 0) {
            ...
            /* Cache serviced amount */
            uint256 servicedAmount = Math.min(item.pendingDeposit, remainingServiceAmount);

            /* Calculate transfer amount */
            uint256 transferAmount = Math.mulDiv(amount, servicedAmount, totalServiceAmount);
    ...
    function _depositAndStake(
        ...
        if (_stakedUsdai.depositSharePrice() > maxDepositSharePrice) revert InvalidSharePrice();
