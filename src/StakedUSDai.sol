    ...
    IERC7540Operator
{
        ...
        )
    {
        return (
            ...
            RedemptionLogic._nextRedemptionTimestamp(_genesisTimestamp)
        );
    }
    ...
    function serviceRedemptions(
        uint256 shares
    ) external onlyRole(STRATEGY_ADMIN_ROLE) nonZeroUint(shares) returns (uint256, uint256) {
        /* Process redemptions */
        (uint256 sharesProcessed, uint256 amountProcessed, bool allRedemptionsServiced) =
        ...
        emit RedemptionsServiced(sharesProcessed, amountProcessed, allRedemptionsServiced);

        return (sharesProcessed, amountProcessed);
    }
