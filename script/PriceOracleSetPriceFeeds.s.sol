...
contract PriceOracleSetPriceFeeds is Deployer {
    function run(address[] memory tokens, address[] memory priceFeeds) public broadcast useDeployment {
        ...
        if (priceOracle.hasRole(0x00, msg.sender)) {
            priceOracle.setTokenPriceFeeds(tokens, priceFeeds);
        } else {
            console.log("\nCalldata");
            console.log("Target:   %s", address(priceOracle));
            console.log("Calldata:");
            console.logBytes(
                abi.encodeWithSelector(ChainlinkPriceOracle.setTokenPriceFeeds.selector, tokens, priceFeeds)
            );
        }
    }
}
