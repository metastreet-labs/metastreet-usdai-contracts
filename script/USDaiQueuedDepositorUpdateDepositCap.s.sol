...
contract USDaiQueuedDepositorUpdateDepositCap is Deployer {
    function run(uint32 srcEid, uint256 depositCap, bool resetCounter) public broadcast useDeployment {
        ...
        if (usdaiQueuedDepositor.hasRole(0x00, msg.sender)) {
            usdaiQueuedDepositor.updateDepositCap(srcEid, depositCap, resetCounter);
        } else {
            console.log("\nCalldata");
            console.log("Target:   %s", address(usdaiQueuedDepositor));
            console.log("Calldata:");
            console.logBytes(
                abi.encodeWithSelector(USDaiQueuedDepositor.updateDepositCap.selector, srcEid, depositCap, resetCounter)
            );
        }
    }
}
