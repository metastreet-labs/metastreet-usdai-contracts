...
usage() {
    ...
    echo "  deploy-usdai-queued-depositor <deployer> <multisig> <whitelisted tokens> <min amounts> <deposit caps>"
...
case $1 in
   ...
   "deploy-usdai-queued-depositor")
        ...
        run "$NETWORK" "${NETWORK^^}_RPC_URL" "script/DeployUSDaiQueuedDepositor.s.sol:DeployUSDaiQueuedDepositor" --sig "run(address,address,address[],uint256[],uint256[])" $2 $3 "$4" "$5" "$6"
        ;;
