...
contract QEVRegistryTest is QEVBaseTest {
    ...
    function test__AuctionBasicLifecycle_InitialAuctionSetup() public {
        ...
        assertEq(
            auctionId,
            IStakedUSDai(address(stakedUsdai)).redemptionTimestamp(),
            "Initial auction ID should be redemption timestamp"
        );

        (uint256 bidCount, uint256 processedBidCount, uint256 processedRedemptionId) = qevRegistry.auction(0);
    ...
    function test__AuctionBasicLifecycle_CannotSettleEmptyAuction() public {
        uint64 auctionId = qevRegistry.nextAuctionId();

        advanceAuctionTime(auctionId);

        // Test: Cannot settle empty auction
        vm.expectRevert(IQEVRegistry.InvalidAuctionStatus.selector);
        vm.prank(users.manager);
        qevRegistry.settleAuction(auctionId);
    }

    function test__AuctionBasicLifecycle_CannotSettleEmptyInvalidAuctionId() public {
        ...
        // Test: Cannot settle auction ID smaller than previous auction ID
        vm.expectRevert(IQEVRegistry.InvalidAuctionStatus.selector);
        vm.prank(users.manager);
        qevRegistry.settleAuction(auctionId - 1);
    }
    ...
    function test__BidValidation_BasisPointTooLow() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IQEVRegistry.InvalidBasisPoint.selector, 0));
        vm.prank(users.manager);
        qevRegistry.postBids(auctionId, signedBids);
    }
    ...
    function test__BidValidation_ZeroRedemptionAmount_WithServicedRedemption() public {
        // Test: Completely serviced redemptions are skipped in posting bids
        ...
        signedBids[1] = createSignedBid(auctionId, redemptionId2, 100_000 ether, 2000, user2PrivateKey);
    ...
    function test__BidValidation_DuplicateBid() public {
        ...
        vm.prank(users.manager);
        qevRegistry.postBids(auctionId, secondBid);
    }
    ...
    function test__MultipleBatches_PostBidsInMultipleBatches() public {
        ...
        vm.prank(users.manager);
        qevRegistry.postBids(auctionId, batch1);
        ...
        vm.prank(users.manager);
        qevRegistry.postBids(auctionId, batch2);
    ...
    function test__NonceManagement_CannotIncrementAfterAuctionEnd() public {
        ...
        advanceAuctionTime(auctionId + 1);
