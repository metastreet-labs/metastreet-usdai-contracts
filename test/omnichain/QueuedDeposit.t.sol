...
contract USDaiQueuedDepositTest is OmnichainBaseTest {
    ...
    function test__USDaiQueuedDeposit_RevertWhen_DepositCapExceeded() public {
        usdaiQueuedDepositor.setDepositCap(10_000_000 * 1e18);

        (uint256 cap, uint256 total) = usdaiQueuedDepositor.depositCap();
        ...
        usdaiQueuedDepositor.setDepositCap(cap + 1);

        vm.startPrank(user);
        usdaiQueuedDepositor.deposit(IUSDaiQueuedDepositor.QueueType.Deposit, address(usdtHomeToken), cap + 1, user, 0);
        vm.stopPrank();

        (, total) = usdaiQueuedDepositor.depositCap();
        assertEq(total, cap + 1);
    }
}
